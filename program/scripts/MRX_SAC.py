...
class ObservationEncoder:
    ...
    def encode(self, game_state: Dict[str, Any], players: List[Dict[str, Any]],
               possible_moves: Optional[List[Dict[str, Any]]] = None, graph: Optional[Dict[str, Any]] = None):
        obs: List[float] = []
...
class DiscreteSACAgent:
    def __init__(self, state_dim: int, action_dim: int, model_path: Optional[str] = None):
        ...
        if model_path is None:
            ...
            model_path = os.path.join(script_dir, "sac_mrx.pth")
        ...
        if os.path.exists(self.model_path):
            ...
            print(f"[AI] Wczytano SAC z: {self.model_path} (epsilon={self.epsilon:.3f}, steps={self.steps})")
    ...
    def update(self):
        ...
        # --- Policy update (fresh forward, after Q step) ---
...
sock.bind("tcp://*:5555")
print("AI SAC Server listening on 5555")
...
try:
    while True:
        ...
        if "[GameOver]" in raw or "winner" in req or "winner" in raw:
            winner = req.get("winner", "Detectives" if "Detectives" in raw else "MrX")
            ...
            log_game(winner=winner, mrx_reward=total_reward, rounds=current_round)
            if agent:
        ...
        if agent is None:
            ...
            print(f"[INIT] SAC agent ready. Obs dim={obs.size}")
    ...
    print("[STOP] Serwer zatrzymany przez użytkownika.")
finally:
    sock.close()
    ctx.term()
