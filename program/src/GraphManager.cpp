...
Edge::Edge(int i_Type_, Node* p_A, Node* p_B)
    : i_Type(i_Type_), m_GeomType(GeometryType::None)
{
    p_Endpoints[0] = p_A;
    p_Endpoints[1] = p_B;
}

Node* Edge::OtherNode(const Node* p_Me) const {
    if (p_Me == p_Endpoints[0]) return p_Endpoints[1];
    if (p_Me == p_Endpoints[1]) return p_Endpoints[0];
    return nullptr;
}

Edge::GeometryType Edge::GetGeometryType() const {
    return m_GeomType;
}

bool Edge::HasGeometry() const {
    return !m_vec_PolylineNorm.empty();
}

bool Edge::SetPolylineNormalized(const std::vector<Vec2>& vec_Points) {
...
const std::vector<Vec2>& Edge::GetPolylineNormalized() const {
    return m_vec_PolylineNorm;
}

void Edge::ClearGeometry() {
    m_vec_PolylineNorm.clear();
    m_GeomType = GeometryType::None;
}

Node::Node(int i_Id_, int i_X_, int i_Y_, bool b_Special)
    : i_Id(i_Id_), i_X(i_X_), i_Y(i_Y_)
{
}

bool Node::ConnectTo(Node* p_Other, int i_Type) {
    if (!p_Other) return false;

    auto s_Edge = std::make_shared<Edge>(i_Type, this, p_Other);
    m_vec_Slots.push_back({s_Edge});
    p_Other->m_vec_Slots.push_back({s_Edge});
    return true;
}

void Node::RemoveEdge(Edge* p_Edge) {
    for (auto it = m_vec_Slots.begin(); it != m_vec_Slots.end(); ) {
        if (it->s_Edge.get() == p_Edge) {
...
Node* Node::OtherNode(int i_SlotIndex) const {
    if (i_SlotIndex < 0 || i_SlotIndex >= static_cast<int>(m_vec_Slots.size())) return nullptr;
    const Slot& slot = m_vec_Slots[i_SlotIndex];
    if (!slot.s_Edge) return nullptr;
    return slot.s_Edge->OtherNode(this);
}

Edge* Node::GetEdge(int i_SlotIndex) const {
    if (i_SlotIndex < 0 || i_SlotIndex >= static_cast<int>(m_vec_Slots.size())) return nullptr;
    return m_vec_Slots[i_SlotIndex].s_Edge.get();
}

int Node::GetSlotCount() const {
    return static_cast<int>(m_vec_Slots.size());
}

int Node::GetSlotType(int i_SlotIndex) const {
    if (i_SlotIndex < 0 || i_SlotIndex >= static_cast<int>(m_vec_Slots.size())) return 0;
    const Slot& slot = m_vec_Slots[i_SlotIndex];
    if (!slot.s_Edge) return 0;
    return slot.s_Edge->i_Type;
}

int Node::ConnectionCount() const {
    int i_Count = 0;
    for (const auto& slot : m_vec_Slots) {
        if (slot.s_Edge) ++i_Count;
    }
    return i_Count;
}

std::vector<Node*> Node::GetNeighborsWithType(int i_Type) const {
    std::vector<Node*> vec_Neighbors;
    for (const auto& slot : m_vec_Slots) {
        if (slot.s_Edge && slot.s_Edge->i_Type == i_Type) {
            Node* p_Other = slot.s_Edge->OtherNode(this);
...
GraphManager::GraphManager(int i_MaxNodes)
    : u_Nodes(std::make_unique<Node[]>(i_MaxNodes + 1)),
...
std::string GraphManager::Trim(const std::string& s_Str) {
...
int GraphManager::TransportTypeFromString(const std::string& s_TypeStr) {
...
Edge* GraphManager::FindEdgeOneWay(int i_A, int i_B, int i_Type) {
    if (!IsValidNode(i_A) || !IsValidNode(i_B)) return nullptr;
    Node* p_NodeA = &u_Nodes[i_A];
...
void GraphManager::LoadNodeData(const std::string& s_Filename, bool b_Verbose) {
    ...
    while (std::getline(file, s_Line)) {
        ...
        if (IsValidNode(i_Id)) {
            u_Nodes[i_Id].i_X = std::stoi(s_XStr);
            u_Nodes[i_Id].i_Y = std::stoi(s_YStr);
...
void GraphManager::LoadConnections(const std::string& s_Filename, bool b_Verbose) {
    std::ifstream file(s_Filename);
    if (!file.is_open()) {
    ...
    while (std::getline(file, s_Line)) {
        ...
        if (IsValidNode(i_Src) && IsValidNode(i_Dst) && i_Type > 0) {
            u_Nodes[i_Src].ConnectTo(&u_Nodes[i_Dst], i_Type);
...
bool GraphManager::SetEdgePolylineNormalized(int i_Src, int i_Dst, int i_TransportType, const std::vector<Vec2>& vec_PointsNorm) {
...
bool GraphManager::LoadEdgeGeometryCSV(const std::string& s_Filename, bool b_Verbose) {
    std::ifstream file(s_Filename);
    if (!file.is_open()) {
    ...
    while (std::getline(file, s_Line)) {
        ...
        try {
            ...
            if (IsValidNode(i_Src)) {
                vec2_StartNorm.f_X = (f_GridMaxX > 0.0f) ? (u_Nodes[i_Src].i_X / f_GridMaxX) : 0.0f;
                vec2_StartNorm.f_Y = (f_GridMaxY > 0.0f) ? (u_Nodes[i_Src].i_Y / f_GridMaxY) : 0.0f;
            }
            if (IsValidNode(i_Dst)) {
                vec2_EndNorm.f_X = (f_GridMaxX > 0.0f) ? (u_Nodes[i_Dst].i_X / f_GridMaxX) : 0.0f;
                vec2_EndNorm.f_Y = (f_GridMaxY > 0.0f) ? (u_Nodes[i_Dst].i_Y / f_GridMaxY) : 0.0f;
            }
...
void GraphManager::LoadData(const std::string& s_PosFile, const std::string& s_ConFile, bool b_Verbose) {
...
int GraphManager::GetBoundsX(int i_NumNodes) {
    int i_MaxX = 1;
    for (int i = 1; i <= i_NumNodes; ++i) {
        if (u_Nodes[i].i_X > i_MaxX) i_MaxX = u_Nodes[i].i_X;
    }
    return i_MaxX;
}

int GraphManager::GetBoundsY(int i_NumNodes) {
    int i_MaxY = 1;
    for (int i = 1; i <= i_NumNodes; ++i) {
        if (u_Nodes[i].i_Y > i_MaxY) i_MaxY = u_Nodes[i].i_Y;
    }
    return i_MaxY;
}

Node* GraphManager::GetNode(int i_Id) {
...
const Node* GraphManager::GetNode(int i_Id) const {
    if (i_Id < 1 || i_Id > m_i_NodeCount) {
        return nullptr;
    }
    return &u_Nodes[i_Id];
...
std::vector<Node*> GraphManager::GetNeighbors(int i_NodeId) {
...
std::vector<GraphManager::Connection> GraphManager::GetConnections(int i_NodeId) const {
...
std::vector<Node*> GraphManager::GetNeighborsByType(int i_NodeId, int i_Type) {
    Node* p_Node = GetNode(i_NodeId);
...
bool GraphManager::IsValidNode(int i_Id) const {
