...
PythonBridge::~PythonBridge() {
  ...
  // First, try to join the worker thread with a timeout
  // This is safer than detach because we ensure thread exits before socket
  // cleanup
  if (worker_.joinable()) {
    // Create a flag to track if join completed
    std::atomic<bool> joinCompleted{false};

    // Spawn a helper thread to do the join (so we can timeout)
    std::thread joiner([this, &joinCompleted]() {
      try {
        worker_.join();
        joinCompleted = true;
      } catch (...) {
    ...
    // Wait up to 500ms for the join to complete
    auto start = std::chrono::steady_clock::now();
    while (!joinCompleted && std::chrono::steady_clock::now() - start <
                                 std::chrono::milliseconds(500)) {
      cv_.notify_all(); // Keep nudging the worker
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }

    if (joiner.joinable()) {
      if (joinCompleted) {
        joiner.join(); // Clean join completed
      } else {
        // Join didn't complete in time - detach as fallback
        joiner.detach();
        // Give extra time for worker to notice stop_ flag
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
      }
    }
  }

  // Close socket first (before context) - must be done AFTER worker exits
  try {
    socket_.set(zmq::sockopt::linger, 0);
    socket_.close();
  } catch (...) {
...
PythonBridge *PythonBridge::Instance() { return s_instance; }

std::future<nlohmann::json>
PythonBridge::sendRequestAsync(const nlohmann::json &req) {
  Job j;
  j.req = req;
...
void PythonBridge::workerThread() {
  while (true) {
    ...
    if (stop_) {
      ...
      try {
        job.prom.set_exception(std::make_exception_ptr(
            std::runtime_error("Bridge shutting down")));
      } catch (...) {
      }
      continue;
    }
    ...
    try {
      ...
      if (!recv_result || stop_) {
        job.prom.set_exception(std::make_exception_ptr(
            std::runtime_error("Recv timeout or shutdown")));
